...
class SemanticEntity(object):
    def __init__(self, tag, corrector, text):
...
class SemanticProperty(object):
    def __init__(self, tag, text, entity_map=None):
        self.id = int(tag["xmi:id"])
...
def parse(filepath):
    ...
    entities = [SemanticEntity(tag, corrector, text) for tag in xml.find_all("custom:SemanticEntities")]
    print(f"Parsed {len(entities)} Entities")
    
    entity_map = {e.id:e for e in entities}
    properties = [SemanticProperty(tag, text, entity_map) for tag in xml.find_all("custom:SemanticRelations")]
