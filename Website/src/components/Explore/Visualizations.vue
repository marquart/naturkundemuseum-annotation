<template>
  <div>
    ...
    <p>
      Through the interface below you can navigate the graph structure of our
      semantic web. Clicking the box of an entity displays the immediate
      neighborhood of that entity in the graph. You can jump to the location of
      the selected entity in the text by clicking the button "<img
        class="symbol"
        src="/icons/book.svg"
        alt="Book Symbol"
      />
      Show Entity in Text". If you want to start exploring the graph from a
      random node, click on the "<img
        class="symbol"
        src="/icons/dice-solid.svg"
        alt="Dice Symbol"
      />
      Start with random Entity" button. The entity which is currently in focus
      is displayed with <span class="rounded">rounded corners</span> (and not
      clickable), every other annotated entity in its neighborhood is displayed
      with <span class="sharp">sharp corners</span> (and clickable to set it in
      focus).
    </p>
    ...
    <div class="navigationElements">
      ...
      >
        <img class="symbol" src="/icons/book.svg" alt="Book Symbol" /> Show Entity in
        Text
      </div>
      <div class="navigationButton functionalButton" @click="setRandomEntityID">
        <img class="symbol" src="/icons/dice-solid.svg" alt="Dice Symbol" /> Go to
        random Entity
      </div>
    </div>
...
function setRandomEntityID() {
  if (entities.value.length > 0) {
    ...
    router.push({ name: "explore", params: { id: entities.value[idx].id } });
  } else {
